...
/obj/projectile
	...
	var/damage = 10
	var/damage_type = SHOT //Many damage types can be here.
...
/obj/projectile/proc/on_hit(atom/target, blocked = FALSE, pierce_hit)
	...
	if(blocked != 100) // not completely blocked
		if (damage && L.blood_volume && IS_PHYSICAL_DAMAGE(damage_type))
...
/obj/projectile/proc/can_hit_target(atom/target, direct_target = FALSE, ignore_loc = FALSE, cross_failed = FALSE)
	...
	if(target.density || cross_failed) //This thing blocks projectiles, hit it regardless of layer/mob stuns/etc.
		if (isliving(target)) //But not if we respect allegiance!
			if (no_friendly_fire && faction_check(faction, target:faction))
				return FALSE
		return TRUE
	if(!isliving(target))
