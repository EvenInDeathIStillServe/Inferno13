...
/obj/projectile/beam/beam_rifle/hitscan/generate_hitscan_tracers(cleanup = TRUE, duration = 5, impacting = TRUE, highlander)
	...
	if(highlander && istype(gun))
		...
		if (melee_weapon)
			...
			for(var/datum/point/p in beam_segments)
				melee_weapon.current_indicators += generate_tracer_between_points(p, beam_segments[p], tracer_type, color, duration, hitscan_light_range, hitscan_light_color_override, hitscan_light_intensity)
		if (gun)
			QDEL_LIST(gun.current_indicators)
			for(var/datum/point/p in beam_segments)
				gun.current_indicators += generate_tracer_between_points(p, beam_segments[p], tracer_type, color, duration, hitscan_light_range, hitscan_light_color_override, hitscan_light_intensity)
		else for(var/datum/point/p in beam_segments)
