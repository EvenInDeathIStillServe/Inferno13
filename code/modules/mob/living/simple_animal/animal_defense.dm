...
/mob/living/simple_animal/attack_hand(mob/living/carbon/human/user, list/modifiers)
	...
	if(!user.combat_mode)
		if (stat == DEAD)
			user.visible_message(span_notice("[user] rifles through [src]."), \
						span_notice("You rifle through [src]."))
			if (deep_loot.len)
				...
				deep_loot = list()
			if (cash_loot_max)
				to_chat(user, span_notice("You find an anonymous credit holochip that you appropriate."))
				user.payact(rand(cash_loot_min,cash_loot_max))
				cash_loot_max = 0
			return
