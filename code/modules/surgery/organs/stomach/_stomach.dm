...
/obj/item/organ/stomach/on_life(delta_time, times_fired)
	...
	if(istype(owner, /mob/living/carbon/human))
		...
		if(!(organ_flags & ORGAN_FAILING))
			...
			handle_thirst(humi, delta_time, times_fired)
...
/obj/item/organ/stomach/proc/handle_hunger(mob/living/carbon/human/human, delta_time, times_fired)
	...
	switch(human.nutrition)
		...
		if(NUTRITION_LEVEL_PECKISH to NUTRITION_LEVEL_FULL)
			human.clear_alert("nutrition")
		if(NUTRITION_LEVEL_HUNGRY to NUTRITION_LEVEL_PECKISH)
			human.throw_alert("nutrition", /atom/movable/screen/alert/hunger, 1)
		if(NUTRITION_LEVEL_MALNOURISHED to NUTRITION_LEVEL_HUNGRY)
			human.throw_alert("nutrition", /atom/movable/screen/alert/hunger, 2)
		if(NUTRITION_LEVEL_STARVING to NUTRITION_LEVEL_MALNOURISHED)
			human.throw_alert("nutrition", /atom/movable/screen/alert/hunger, 3)
		if(0 to NUTRITION_LEVEL_STARVING)
			human.throw_alert("nutrition", /atom/movable/screen/alert/hunger, 4)

/obj/item/organ/stomach/proc/handle_thirst(mob/living/carbon/human/human, delta_time, times_fired)
	if(HAS_TRAIT(human, TRAIT_NOTHIRST))
		return
	// hydration decrease
	if (human.hydration > 0 && human.stat != DEAD)
		// THEY THIRST
		var/thirst_rate = THIRST_FACTOR * human.physiology.thirst_mod
		human.adjust_hydration(-thirst_rate * delta_time)
	switch(human.hydration)
		if(HYDRATION_LEVEL_SLAKED to INFINITY)
			human.clear_alert("hydration")
		if(HYDRATION_LEVEL_THIRSTY to HYDRATION_LEVEL_SLAKED)
			human.throw_alert("hydration", /atom/movable/screen/alert/thirst, 1)
		if(HYDRATION_LEVEL_PARCHED to HYDRATION_LEVEL_THIRSTY)
			human.throw_alert("hydration", /atom/movable/screen/alert/thirst, 2)
		if(HYDRATION_LEVEL_DEADLY to HYDRATION_LEVEL_PARCHED)
			human.throw_alert("hydration", /atom/movable/screen/alert/thirst, 3)
		else
