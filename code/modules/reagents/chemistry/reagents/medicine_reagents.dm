...
/datum/reagent/medicine/nanite/on_mob_life(mob/living/carbon/M, delta_time, times_fired)
	if(M.oxyloss > 35)
		M.setOxyLoss(35, 0)
...
/datum/reagent/medicine/nanite/expose_mob(mob/living/exposed_mob, methods=TOUCH, reac_volume, show_message = TRUE)
	. = ..()
	if(!iscarbon(exposed_mob) || (exposed_mob.stat == DEAD))
		return

	if(methods & (INGEST|VAPOR))
		exposed_mob.adjustToxLoss(3.75*reac_volume*REM)
		if(show_message)
